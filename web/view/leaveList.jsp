	    ...
	    $('#dataList').datagrid({
	        ...
	        columns: [[
				...
				{field:'studentId',title:'学号',width:150, sortable:false},
 		        ...
 		        {field:'date',title:'请假开始时间',width:240,
					// formatter: function(value,row,index){
					//
					// 	console.log(value);
					//
 		        ...
 		        {field:'over_date',title:'请假截止时间',width:240,
					// formatter: function(value,row,index){
					//
 		        	// console.log(value);
					// 	if (row.user){
	    	...
	    	buttons: [
	    		{
					...
					handler:function(){
						...
						if(!validate){
							...
							var date= $("#dataList").datagrid("getSelected").date;
							var over_date=$("#dataList").datagrid("getSelected").over_date;

							var data = {id:id, studentid:studentid, info:info,remark:remark,status:status,date:date,over_date:over_date};
...
<body>
	...
	<!-- 工具栏 -->
	<div id="toolbar">
		<div style="float: left;"><a id="add" href="javascript:;" class="btn btn-success" data-options="iconCls:'icon-add',plain:true">
            <i class="bi bi-plus-circle-fill"></i>

            添加</a></div>
		<c:if test="${userType == 2}">
		<div style="float: left;"><a id="edit" href="javascript:;" class="btn btn-info" data-options="iconCls:'icon-edit',plain:true">
            <i class="bi bi-pencil-square"></i>

            修改</a></div>
		</c:if>
		<c:if test="${userType == 1 || userType == 3}">
		<div style="float: left;margin-left: 10px"><a id="check" href="javascript:;" class="btn btn-warning" data-options="iconCls:'icon-edit',plain:true">
            <i class="bi bi-person-bounding-box"></i>
            审核</a></div>
		</c:if>
		<div style="float: left;  margin-left: 10px"><a id="delete" href="javascript:;" class="btn btn-danger" data-options="iconCls:'icon-some-delete',plain:true">
		...
		<div style="margin-top: 3px; display: flex;">
            ...
            <input id="studentList" class="easyui-textbox" name="studentid" />
	...
	<div id="addDialog" style="padding: 10px">
    	<form id="addForm" method="post">
	    	<table cellpadding="8" >
				...
				<tr >
					<td>请假开始日期:</td>
					<td  class="date"  style="width:298px;height:158px;"></td>


				</tr>
				<tr >
					<td>请假截止日期:</td>
					<td class="over_date"   style="width:298px;height:158px;"></td>

				</tr>

	    	</table>
			<input id="mydate" style="display: none" name="date">
			<input id="myover_date" style="display: none" name="over_date">
	    </form>
	</div>
	...
	$(document).ready(function(){
		$('.date').calendar({
			...
			onSelect: function(date){
				$("#mydate").val(date.toLocaleDateString()+new Date().toLocaleTimeString())
		...
		$('.over_date').calendar({
			...
			// fit:true,
			onSelect: function(date){
				// $("#myover_date").val(date.toLocaleDateString()+new Date().toLocaleTimeString())
				$("#myover_date").val(date.toLocaleDateString()+"晚上10:00:00")
